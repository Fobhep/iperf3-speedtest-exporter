...
# Create metrics
download_speed = Gauge('speedtest_download_Mbps',
                       'Speedtest current Download Speed in Mb/s')
upload_speed = Gauge('speedtest_upload_Mbps',
                     'Speedtest current Upload speed in Mb/s')
...
def run_tests():
    ...
    app.logger.info(
        ...
        + " Download[Mbps]: " + str(r_download_speed)
        + " | Upload[Mbps]: " + str(r_upload_speed))
    return make_wsgi_app()


@app.route("/")
def mainPage():
    return ("<h1>Welcome to Iperf3-Speed-Exporter.</h1>" +
...
if __name__ == '__main__':
    ...
    app.logger.info(
        "Starting Speedtest-Exporter on http://localhost:" + str(PORT))
